...
const createPublishersAccount = async (req, res) => {
  const hashedPassword = await bcrypt.hash(req.body.password, 10);
  const { name, email, username, address, join_date, phone_number } = req.body;
  const sql = `Insert into publishers (name, password, email, username, address, join_date, phone_number) 
  values (?,?,?,?,?,?,?)`;
...
const logInAccount = (req, res) => {
  ...
  const sql1 = `select * from publishers where username =?`;
...
const deletePublishersAccount = (req, res) => {
  const { id } = req.params;
  const sql2 = `delete from publishers where id = ?`;
...
module.exports = {
  createPublishersAccount,
  logInAccount,
  deletePublishersAccount,
};
